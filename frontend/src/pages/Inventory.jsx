...
export default function Inventory() {
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      setBulkError(err?.response?.data?.message || "Import failed. Please check the file and try again.");
    } finally {
  ...
  return (
    <Layout>
      ...
      {showBulkImport && (
        ...
        >
          <div className="card" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 420, width: "90%" }}>
            <h3 style={{ marginTop: 0 }}>Bulk Import Diamonds</h3>
            <p style={{ fontSize: 14, color: "#666" }}>
              Upload an Excel file (.xlsx) with columns: carat, cut, color, clarity, shape, certification, price, stockQuantity.
            </p>

            <input type="file" accept=".xlsx,.xls" onChange={handleBulkFileChange} style={{ marginBottom: 12 }} />

            {bulkError && (
              <div style={{ color: "#c00", fontSize: 14, marginBottom: 12 }}>{bulkError}</div>
            )}

            {bulkResult && (
  <div style={{ fontSize: 14, marginBottom: 12 }}>
    <div style={{ color: "green" }}>✔ {bulkResult.importedCount} diamond(s) imported</div>
    {bulkResult.skippedCount > 0 && (
      <div style={{ color: "#b8860b", marginTop: 6 }}>
        ⚠ {bulkResult.skippedCount} duplicate(s) skipped
      </div>
    )}
    {bulkResult.failedCount > 0 && (
                  <div style={{ color: "#c00", marginTop: 6 }}>
                    ✘ {bulkResult.failedCount} row(s) failed:
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              ...
              <button className="btn btn-secondary" onClick={closeBulkImport}>Close</button>
            </div>
          </div>
        </div>
      )}
